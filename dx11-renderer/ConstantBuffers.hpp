...
template <typename C> class ConstantBuffer : public Bindable
{
  ...
  public:
    ConstantBuffer( Graphics& gfx, const C& consts, UINT _slot = 0 ) : slot( _slot )
    {
    ...
    ConstantBuffer( Graphics& gfx, UINT _slot = 0 ) : slot( _slot )
    {
        INFOMAN( gfx );
        D3D11_BUFFER_DESC cbd;
        cbd.BindFlags           = D3D11_BIND_CONSTANT_BUFFER;
...
template <typename C> class VertexConstantBuffer : public ConstantBuffer<C>
{
  ...
  public:
    ...
    {
        return Codex::Get().Resolve<VertexConstantBuffer>( gfx, consts, slot );
    }
