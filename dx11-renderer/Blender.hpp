...
namespace Bind
{
	...
	public:
		Blender(Graphics & gfx, bool isBlending, std::optional<float> factors_in)
			:
			_isBlending(isBlending)
		{
			if (factors_in)
			{
				_factors.emplace();
		...
		static std::string GenerateUID(bool isBlending, std::optional<float> factors_in)
		{
			...
			return typeid(Blender).name() + (isBlending ? "ON"s : "OFF"s ) + (factors_in ? "#f"s + std::to_string(*factors_in) : "");
		}
