...

{
    ...
    const auto pScene = importer.ReadFile (
        modelPath.c_str (), aiProcess_Triangulate | aiProcess_JoinIdenticalVertices | aiProcess_ConvertToLeftHanded |
                                aiProcess_GenNormals | aiProcess_CalcTangentSpace );
    const size_t last_slash_idx = _assetLocation.rfind ( '\\' ) != std::string::npos ? _assetLocation.rfind ( '\\' )
                                                                                     : _assetLocation.rfind ( '/' );
...
std::unique_ptr<Mesh> Model::ParseMesh ( Graphics& gfx, const aiMesh& mesh, aiMaterial* const* materials, float scale )
{
    ...
    if ( hasDiffuseMap && hasNormalMap && hasSpecularMap )
    {
        ...
        bindables.push_back (
            PixelShader::Resolve ( gfx, hasAlpha ? "PhongPSSpecNormMask.cso" : "PhongPSNormalSpecMap.cso" ) );
